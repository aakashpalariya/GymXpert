...
export default function AddGym() {
    ...
    const getGymDetails = async () => {
        let res = await fetch("https://localhost:5001/api/gym/");
    
        if (!res.ok) {
            throw new Error("Failed to fetch gym details");
        }  
        const gymList: Gym[] = (await res.json()).map((gym: any) => ({
            ...gym,
            createdAt: new Date(gym.createdAt) // Convert string to Date
        }));
                                        ...
                                        <TableCell className="px-4 py-3 text-gray-500 text-theme-sm dark:text-gray-400">
                                            <div className="flex">
                                                <div className="px-2 py-2">
                                                    <svg className="cursor-pointer hover:fill-error-500 dark:hover:fill-error-500 fill-gray-700 dark:fill-gray-400" width="20" height="20" fill="none" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
